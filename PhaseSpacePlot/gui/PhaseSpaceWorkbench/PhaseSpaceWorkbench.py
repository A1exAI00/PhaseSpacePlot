...
class PhaseSpaceWorkbench(pswp, pswpo, pswpl, pswd):
    def __init__(self, app):
        ...
        self.trajectories = {}
        self.trajectories[0] = Trajectory(np.zeros(len(self.app.variable_names)))
        return
    ...
    def get_last_state(self, n):
        return np.array([self.trajectories[n].sol[i][-1] for (i, variable_name) in enumerate(self.app.variable_names)])
