...
def callback_add_DS_folder(sender, app_data):
    global DS_filepath, vars_names, pars_names, axis_posible_labels, SaSs, ODEs

    # Get dynamical system folder
    DS_folder_path = app_data["file_path_name"]
    ...
    # List of StateAndSolution objects
    SaSs = []
        ...
        for (i, par_name) in enumerate(pars_names):
            with dpg.group(horizontal=True):
                ...
                par_step_obj = dpg.add_input_float(label=par_name+" step", 
                                                ...
                                                tag=par_name+"_step", 
                                                callback=callback_pars_step_change,
                                                width=100,
                                                step=0.0)
...
with dpg.file_dialog(label="Choose folder with Dynamical System",
                     ...
                     show=True, 
                     callback=callback_add_DS_folder, 
